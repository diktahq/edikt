...
// No LLM invocation, no exec.LookPath, no network — pure Go only.
package sidecardiff
...
// verificationTexts extracts the .Text field from each VerificationEntry,
// so the jaccard comparator can operate on the legacy string-token shape.
// The verify: command is intentionally ignored for similarity scoring —
// only the human-readable text matters for "did the regenerated sidecar
// preserve the verification checklist?"
func verificationTexts(entries []sidecar.VerificationEntry) []string {
...
func tier3VerificationJaccard(exp, act *sidecar.Sidecar, minJaccard float64) TierResult {
	...
	expTokens := greppableTokens(verificationTexts(exp.Verification))
	actTokens := greppableTokens(verificationTexts(act.Verification))
