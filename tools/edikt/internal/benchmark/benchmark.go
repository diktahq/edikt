...
// adversarial benchmark mode introduced in v0.6.0.
//
...
// - JSONL redaction before any write
//
// This package MUST NOT shell out to claude or any LLM CLI.
package benchmark
...
// VerdictSet is the set of evaluator verdicts that count as "held" for
...
// a line that would be written. The caller MUST NOT write the line and MUST
// abort the benchmark run.
var ErrCredentialDetected = errors.New("credential pattern detected in benchmark output — aborting before write")
...
type BenchmarkRecord struct {
	...
	// Verdict is the parsed evaluator verdict.
...
// RedactRecord applies credential redaction to a BenchmarkRecord and returns
