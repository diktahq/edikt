...
func TestFingerprint_RoundTrips(t *testing.T) {
	...
	want := TopicRenderFingerprint([]*sidecar.Sidecar{pairs[0].Sidecar, pairs[1].Sidecar, pairs[2].Sidecar}, "")
	...
	wantAfter := TopicRenderFingerprint([]*sidecar.Sidecar{pairs[0].Sidecar, pairs[1].Sidecar, pairs[2].Sidecar}, "")
	if wantAfter == want {
