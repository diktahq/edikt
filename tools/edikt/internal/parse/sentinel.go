...
type Sentinel struct {
	...
	// — needed by the schema-completeness gate since
	...
	// Hashes written by the per-artifact compile step.
	...
	// Group assignment (optional today; required in v0.7.0).
	...
	// Routing keywords. Indexed for search and for the benchmark's attack
	// corpus; the governance.md routing table they once fed was retired with
	// the tier-3 render (SPEC-011).  edikt-guard:allow
	// Each artifact contributes its signals; gov:compile unions them per
	// topic (preserving insertion order, deduped). The per-artifact compile
	// commands are responsible for generating these from artifact content;
	// gov:compile aggregates without invoking an LLM.
	...
	// Directive-hardening extensions (optional).
...
// replaced by a single newline. Used by source_hash — SHA-256 of the
// artifact body with the directives block excluded, normalized.
...
func parseSentinelBlock(inner string) (Sentinel, error) {
	...
	for _, line := range lines {
		...
		if inBS {
			...
			// SPEC-009 Plan A AC-1.2: parses behavioral_signal sub-block from in-body sentinel.  // edikt-guard:allow
			// Not *.edikt.yaml. KnownFields off intentional — sub-block schema is intentionally permissive (generic map).
	...
	if inBS && len(bsLines) > 0 {
		sub := strings.Join(bsLines, "\n")
		var m map[string]interface{}
		// SPEC-009 Plan A AC-1.2: trailing-flush variant of the same behavioral_signal sub-block parse.  // edikt-guard:allow
		// Not *.edikt.yaml. KnownFields off intentional — see commentary at line 268.
		if err := yaml.Unmarshal([]byte(sub), &m); err == nil && len(m) > 0 {
			s.BehavioralSignal = m
		}
	}
