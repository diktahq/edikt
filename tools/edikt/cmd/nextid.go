...
func emitListing(out io.Writer, root, relDir string, k listingKind) error {
	...
	// Next id is max(existing numbers)+1, never count+1: gaps and duplicate
	// numbers in the corpus made the count drift below the highest id on
	// disk (field bug: "Next SPEC number: SPEC-028" while SPEC-031 existed).
	maxNum := 0
	byNum := map[int][]string{}
	for _, name := range names {
		n, ok := artifactNum(name, k.idPrefix)
		if !ok {
			continue
		}
		if n > maxNum {
		...
		byNum[n] = append(byNum[n], name)
	}
	next := fmt.Sprintf("%s-%03d", k.idPrefix, maxNum+1)
	existing := "(none yet)"
	if len(names) > 0 {
	...
	for _, n := range dupNums {
		body += fmt.Sprintf("WARNING: duplicate %s numbers: %s-%03d (%s)\n",
			k.idPrefix, k.idPrefix, n, strings.Join(byNum[n], ", "))
	}
	fmt.Fprint(out, wrapLive(body))
	return nil
}

// artifactNum parses the numeric id out of "<PREFIX>-NNN..." names. Returns
// ok=false for names whose char after the prefix is not a digit (e.g.
// "INV-template-old") so they don't poison the numbering.
func artifactNum(name, prefix string) (int, bool) {
