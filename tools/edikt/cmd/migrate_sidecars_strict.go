...
// invocations — pure Go string comparison + regex.
//
...
// aggregated manifest. Pure Go — no LLM invocation.
...
func diffSentinelSidecar(mdPath string, sent *parse.Sentinel, sc *sidecar.Sidecar) []StrictItem {
	...
	for i, legacyVerif := range sent.Verification {
		...
		// sc.Verification carries VerificationEntry now; extract .Text to
		// compare against the legacy bare-string form.
		newVerif := ""
		if i < len(sc.Verification) {
			newVerif = sc.Verification[i].Text
		} else if len(sc.Verification) > 0 {
			// Fall back to the last new item.
			newVerif = sc.Verification[len(sc.Verification)-1].Text
		}
		...
		for _, nv := range sc.Verification {
			if strings.Contains(nv.Text, token) {
				covered = true
				break
			}
		}
