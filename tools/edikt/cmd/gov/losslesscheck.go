...
Exit codes:
  0 — clean: at least one artifact was compared and every comparison passed
  ...
  4 — nothing compared (no artifacts, or all skipped) — UNMEASURED, not clean

Exit 0 requires a comparison to have happened. "All artifacts pass" is
vacuously true over an empty set, and a gate that reports clean when it
found nothing to check is worse than no gate.
`,
	Args: cobra.NoArgs,
	// Bridge *exitErr to os.Exit, as the other six gov subcommands do.
	// Without this, every exitErr this command returns collapses to
	// cobra's generic exit 1 — so the documented code 2 (missing baseline
	// dir) had never actually been emitted, and a caller distinguishing
	// "losses found" from "the check could not run" got 1 for both.
	RunE: func(cmd *cobra.Command, args []string) error {
		err := runLosslessCheck(cmd, args)
		exitFromExitErr(err)
		return err
	},
}
...
func runLosslessCheck(cmd *cobra.Command, args []string) error {
	...
	// validate --report-json path. Resolve absolute, refuse
	...
	// SPEC-009 Plan A AC-1.2: loads .edikt/config.yaml paths subset.  // edikt-guard:allow
	...
	// Nothing compared is not clean.
	//
	// The verdict used to be the Failed check alone, so passed=0 exited 0 —
	// the same code as a corpus where every artifact was compared and every
	// one held. The audit found this at skipped=8, total_losses=0, exit 0:
	// a project that verified nothing reporting the migration lossless.
	//
	// The two zero-subject shapes are distinguished because they call for
	// different action, but neither is a pass: a corpus with no artifacts
	// at all is a setup problem, and a corpus whose artifacts were every
	// one skipped is a coverage problem with a named cause per artifact in
	// the report.
	if rpt.Summary.Passed == 0 {
		if rpt.Summary.TotalArtifacts == 0 {
			return &exitErr{code: 4, msg: "UNMEASURED — no artifacts found under the configured " +
				"decisions/invariants/guidelines paths, so nothing was compared against a baseline. " +
				"This is not a clean result; check paths: in .edikt/config.yaml."}
		}
		return &exitErr{code: 4, msg: fmt.Sprintf("UNMEASURED — all %d artifact(s) were skipped and none "+
			"was compared against a baseline, so no loss could have been detected. "+
			"See the per-artifact skip_reason in %s.", rpt.Summary.TotalArtifacts, losslessReportPath)}
	}
	return nil
}
