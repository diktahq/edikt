...
// compile-history.json state file. Validated: must
// stay within the project root and contain no `..`
// segments.
...
// 2 — flag/argument validation error
...
// orphanIDPattern is the allowlist for the `--orphans` argv:
...
Pure Go — no LLM dispatch, no shell-out. Two consecutive invocations
with the same inputs and a pinned timestamp produce byte-equal output.`,
	Args: cobra.NoArgs,
...
func init() {
	compileHistoryCmd.Flags().StringVar(&chOrphans, "orphans", "",
		"comma-separated orphan IDs (e.g. 'ADR-NNN,INV-NNN'). Empty = no orphans.")
...
func runCompileHistory(cmd *cobra.Command, args []string) error {
	...
	// Validate --history-path. Resolve absolute, refuse traversal,
	...
	// Validate --orphans. Reject anything outside the allowlist.
