...
func TestNextID_ADR_CountsAndLists(t *testing.T) {
	...
	got := runAndCapture(t, "adr")
	// max+1, not count+1: with a gap (001, 002, 007) the next id must clear
	// the highest existing number, never collide with it.
	if !strings.Contains(got, "Next ADR number: ADR-008") {
		t.Errorf("expected next ADR-008 (max 007 → +1), got %q", got)
	}
...
// ── max+1 numbering (bok-services field bug: count-based next-id) ────────────

func TestNextID_Spec_MaxPlusOne_WithGapsAndDuplicates(t *testing.T) {
	// 4 spec dirs on disk, but numbers are 001 (×2), 028, 031. A count-based
	// next-id says SPEC-005 — while SPEC-028 and SPEC-031 exist. Next MUST be
	// max+1 = SPEC-032, and duplicate ids MUST be surfaced, not miscounted.
	withProject(t, "paths:\n  specs: docs/product/specs\n", map[string]string{
		"docs/product/specs/SPEC-001-heuristics-full-completion/spec.md": "x",
		"docs/product/specs/SPEC-001-phase-0a-eval-ingestion/spec.md":    "x",
		"docs/product/specs/SPEC-028-alpha/spec.md":                      "x",
		"docs/product/specs/SPEC-031-delta/spec.md":                      "x",
	})
	got := runAndCapture(t, "spec")
	if !strings.Contains(got, "Next SPEC number: SPEC-032") {
		t.Errorf("next must be max+1 (SPEC-032), got %q", got)
	}
...
func TestNextID_ADR_MaxPlusOne_Gap(t *testing.T) {
	// Two files, numbers 001 and 047 — next must be 048, never 003.
	withProject(t, "paths:\n  decisions: docs/architecture/decisions\n", map[string]string{
		"docs/architecture/decisions/ADR-001-first.md": "x",
		"docs/architecture/decisions/ADR-047-last.md":  "x",
	})
	got := runAndCapture(t, "adr")
	if !strings.Contains(got, "Next ADR number: ADR-048") {
		t.Errorf("next must be max+1 (ADR-048), got %q", got)
	}
	if strings.Contains(got, "WARNING: duplicate") {
		t.Errorf("no duplicates present — must not warn, got %q", got)
	}
}

func TestNextID_INV_NonNumericNamesIgnored(t *testing.T) {
	// A non-numeric INV-prefixed file must not poison the numbering.
	withProject(t, "paths:\n  invariants: docs/architecture/invariants\n", map[string]string{
		"docs/architecture/invariants/INV-002-money.md":    "x",
		"docs/architecture/invariants/INV-template-old.md": "x",
	})
	got := runAndCapture(t, "inv")
	if !strings.Contains(got, "Next INV number: INV-003") {
		t.Errorf("non-numeric names must be ignored for numbering, got %q", got)
	}
}
