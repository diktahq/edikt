...
// `edikt doctor`. Accepted ADRs are immutable, so manual directives
// that cite a non-existent ADR file are surfaced as ORPHAN findings.
// The remediation is to fix the ref tag in the sidecar's
// manual_directives list — never to backfill a missing ADR body.
...
// allowlist is intentionally narrow — normalisation runs in
...
func runOrphanManualRefCheck(projectRoot string, w io.Writer) (warns int, ran bool) {
	...
	for _, sr := range sidecars {
		for _, md := range sr.sc.ManualDirectives {
			...
			id := m[1]
			// validate before any filesystem lookup so a
