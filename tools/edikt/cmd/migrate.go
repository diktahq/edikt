...
// M6 — no-op
//
// once this lands, bin/edikt-shell is deleted.
...
var migrateCmd = &cobra.Command{
	Use:   "migrate",
	Short: "Migrate a pre-v0.6.0 flat layout to the versioned layout",
	Long: `Migrate a pre-v0.6.0 flat layout (hooks/ as directory) into the versioned
...
func runMigrate(ediktRoot, claudeRoot string, dryRun, assumeYes, abortOnly bool) error {
	...
	if !needsMigration(ediktRoot) {
		...
		if err != nil {
			...
			// Migration is invoked by install.sh or /edikt:upgrade — the user
			// typically didn't run `edikt migrate` directly. Point them at the
			// slash flow for retry; doctor wraps the binary equivalent.
			fmt.Fprintf(os.Stderr, "error: secondary migration(s) failed — backups at %s/. Re-run /edikt:upgrade in Claude Code to retry, or /edikt:doctor to inspect.\n",
				backupDir)
			return err
		}
		fmt.Println("\nMigration complete.\n\nMigration summary:")
	...
	if secErr != nil {
		...
		fmt.Fprintf(os.Stderr, "error: secondary migration(s) failed — backups at %s/. Re-run /edikt:upgrade in Claude Code to retry.\n", backupDir)
...
func migrateM4CompileSchema(ediktRoot, claudeRoot string, dryRun bool) (bool, error) {
	...
	// 2 OR 3: this asks "is this a post-v0.1.x tree", not "is this exactly
	// schema 2". ADR-059 bumped the value to 3 for the four-surface render, and  edikt-guard:allow
	// the field has always been documented as monotonic — an exact-equality
	// read was never the contract.
	if containsStr(string(data), "compile_schema_version: 2") ||
		containsStr(string(data), "compile_schema_version: 3") {
		return false, nil
	}
...
func migrateM5ConfigAdditions(ediktRoot string, dryRun bool, backupDir *string) (bool, error) {
	...
	for _, key := range missing {
		...
		case "paths":
			extra.WriteString(`
# Added by edikt v0.6.0 migration
paths:
...
// symlink) AND $EDIKT_ROOT/versions does not exist. That signals a pre-v0.6.0
// flat layout.
...
func hasSecondarySignal(ediktRoot, claudeRoot string) bool {
	...
	if data, err := os.ReadFile(gov); err == nil {
		if !containsStr(string(data), "compile_schema_version: 2") &&
			!containsStr(string(data), "compile_schema_version: 3") { // ADR-059  edikt-guard:allow
...
func copyDirFull(src, dst string) error {
	return filepath.Walk(src, func(path string, info os.FileInfo, err error) error {
		...
		if fi.Mode()&fs.ModeSymlink != 0 {
			...
			os.Remove(target)
			// #nosec G122 -- copyDirFull is a single-process copy into edikt's own managed version dirs (e.g. ~/.edikt/versions/); src/dst are internal, not externally controlled, so the Walk-callback TOCTOU is not a realistic threat here.
