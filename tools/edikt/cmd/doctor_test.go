...
func TestDoctorMinimalLayout(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, "versions", "0.6.0", "commands"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(root, "versions", "0.6.0", "templates"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink(
		filepath.Join(root, "versions", "0.6.0", "commands"),
	...
	if err := os.Symlink(
		filepath.Join(root, "versions", "0.6.0"),
	...
	if err := writeLock(root, "0.6.0", "test"); err != nil {
		t.Fatal(err)
	}
