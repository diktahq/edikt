...
```
/edikt:sync              — detect all linters, generate rules for all found
/edikt:sync golangci     — sync only golangci-lint rules
/edikt:sync go           — sync only golangci-lint rules (alias)
/edikt:sync eslint       — sync only ESLint rules
/edikt:sync ruff         — sync only Ruff rules
/edikt:sync python       — sync only Ruff rules (alias)
/edikt:sync rubocop      — sync only RuboCop rules
/edikt:sync ruby         — sync only RuboCop rules (alias)
/edikt:sync prettier     — note: prettier formatting is handled by PostToolUse hook, no rule needed
/edikt:sync biome        — sync only Biome rules
/edikt:sync --dry-run    — show what would be generated without writing files
```
...
```bash
cat .edikt/config.yaml 2>/dev/null || echo "No .edikt/config.yaml found"
```
...
edikt:sync complete
  ...
  Run /edikt:doctor to verify rule health.
  When linter config changes, run /edikt:sync again.
```
...
If your linter config is in a non-standard location, pass the path directly or run /edikt:sync {linter} after navigating to the package.
```
