...
11. **If sentinel block exists:** read the directives verbatim from the block. Also read `paths:` and `scope:` from the block if present. This is the primary path — no extraction, no distillation.
...
13. Analyze all extracted directives and group them by topic. A topic is a domain area — caching, database, multi-tenancy, authentication, file storage, architecture (cross-cutting), etc.

    Grouping rules:
    - Directives from different sources about the same domain go into the same topic file
    - Each directive keeps its source reference (`ref: ADR-008, §Eviction`)
    - If a directive doesn't fit an obvious topic, group it under `architecture.md` (cross-cutting)
    - Invariants are special — they go into `governance.md` (the index), not into topic files
