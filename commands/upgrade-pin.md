...
If `.edikt/config.yaml` does not exist in the cwd or any ancestor:
```
No edikt config found. Run /edikt:init to set up this project.
```
And stop. (The Step 4 recovery message below is the same gate stated in user-facing form.)
...
On exit 0:
```
✅ Pin aligned to active edikt version.

Next: commit the change with `git commit -m "chore(edikt): pin to vX.Y.Z"`.
```
...
If `.edikt/config.yaml` is not found in the cwd or any ancestor (same as the Step 0 gate; documented here for the Recovery surface):
