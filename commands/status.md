...
**Signals detected:**
```bash
# Typed kinds (ADR-050) are written verbatim and lowercase by stop-hook.sh.
# The pre-ADR-050 prose wording ('ADR', 'Doc gap', 'Security') no longer
# appears in the log — match the machine token, not the phrasing.
grep -E 'adr-candidate|doc-gap|security-audit|sidecar-stale|completion-claim' ~/.edikt/session-signals.log 2>/dev/null | grep -v 'AGENT\|RULE_LOADED'
```
...
```
PRD-NNN accepted → SPEC-NNN accepted → artifacts 3/3 accepted → PLAN-NNN in progress
```
