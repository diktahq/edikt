...
Read the guideline's co-located `<name>.edikt.yaml` sidecar. If the sidecar exists, score each compiled directive for LLM compliance. For each directive in `directives[]` AND `manual_directives[]`, score on:
...
### 3c. Skipped Soft-Rule Report

The sidecar-extractor deliberately does NOT promote soft-language bullets (no normative verb) into directives — so a guideline rule phrased only with `should` / `ideally` / `prefer` / `try to` / `might` / `consider` is silently absent from the compiled sidecar and is **not enforced**. Pre-v0.6 the in-body compiler surfaced these skips inline; restore that visibility here, since the locked extractor's single-line output cannot.

Scan the guideline prose body for bullet rules whose only modality is a soft marker AND which have no corresponding entry in the sidecar's `directives[]` (coarse match: ≥60% token overlap or `source_excerpts[]` line-range overlap). For each such skipped soft rule, emit:

```
⊘ Skipped (soft language → not compiled): "{bullet text}" (line {n})
  Why: no MUST / NEVER / ALWAYS — the extractor does not compile soft rules into directives, so this rule is not enforced.
  Harden: "{suggested MUST/NEVER rewrite}" then run /edikt:guideline:compile {slug}.
```

If every soft bullet was either hardened or already has a directive, emit:

```
✓ No unenforced soft-language rules — every guideline rule compiled into a directive.
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  Unenforced soft rules (skipped by compile): {n}
...
3. **Missing directives (prose → sidecar).** Scan the entire body for imperative sentences (MUST / MUST NOT / NEVER / ALWAYS / SHOULD) not represented in the sidecar's `directives[].text`. For any unrepresented:
   ...
   ```
   Soft-language bullets (no normative verb) are not directives — they are skipped both in extraction and in this check. Use coarse matching (≥60% token overlap or `source_excerpts[]` line-range overlap), not exact-string equality.
