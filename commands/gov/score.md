...
Read the render manifest at `.claude/rules/governance/manifest.yaml` and score exactly the surfaces it lists: the ambient core, the topic files, the skill packages, and the directive index. Do not glob a directory — a walk finds orphans left by a renamed topic and misses anything rendered elsewhere, so the score would describe a set nobody rendered.

The DATA PLANE is the source of truth for what SHOULD be delivered: the sidecars under the configured governance dirs. The manifest says where it landed. Scoring the rendered surfaces alone cannot see a directive that reached no surface at all, which is the failure mode worth catching.

If the manifest is absent, say so and score nothing rather than falling back to a directory walk.
...
- **Manual directive** — flagged by the inline `*(manual)*` marker in the directives region, or by presence in the `[edikt:manual:start]…[edikt:manual:end]` region. Manual directives MUST be counted in the directive total — they are first-class.
- **Prohibition** — appears in a topic file's `[edikt:prohibitions:start]…[edikt:prohibitions:end]` region. Counted in the rubric as a separate metric (see step 4b).
...
Count total tokens across the AMBIENT surfaces only — the ambient core plus anything else loaded on every edit. Trigger-loaded skills and write-time index entries are not ambient cost and counting them would report a budget nobody pays. Use approximate word-to-token ratio of 1.3, and say in the output that the number is an estimate over that ratio, not a tokenizer measurement.
