...
This sub-procedure runs three static quality checks over a parsed directive and returns a list of warnings. It covers three checks: length vs canonical_phrases, phrase-not-in-body, and no-directives reason validation.
...
|---|---|---|
| `adr_id` | string | The ADR or INV identifier (e.g. `ADR-NNN`, `INV-NNN`) |
...
**v0.6.0 grace period:** this check is warn-only. No compile or review is blocked solely because of this warning.

## Check B — Canonical phrase substring match
...
## Check C — no-directives reason validator
...
The three checks are implemented in `bin/edikt gov directive-check` (Go, pure deterministic — see `tools/edikt/internal/dircheck/`).  + ADR-NNN this is an authorized tier-2 orchestration call; the helper is LLM-agnostic.
...
**Exit codes:**
- `0` — checks ran (warning lines on stdout if any). NEVER blocks a caller (grace period).
- `2` — INV-NNN refusal: malformed JSON payload, unknown fields, or empty stdin.

The Go implementation matches the previous Python heredoc byte-for-byte; downstream callers read warnings as substring matches. Callers MUST NOT parse stdout structure.
...
Compile context:
  ...
  Exit 0 even when warnings are present (grace period).
