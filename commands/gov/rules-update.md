...
Resolve the template root, project-mode first (same order as `/edikt:upgrade` §0y-bis — never a hardcoded global path):
...
Rule templates live at `$EDIKT_TEMPLATES/rules/`. Also read `_registry.yaml` from the same directory.

If `EDIKT_TEMPLATES` is empty or templates are not found:
```
...
4. If conflicts found, report them:
   ...
   ⚠ Conflict detected in {pack_name}.md update:
     ...
     Compiled: "Use SELECT * for audit log queries to capture all columns"
