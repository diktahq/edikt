...
This document defines the domain signal table and specialist spawning procedure used by `/edikt:sdlc:plan`, `/edikt:sdlc:code-review`, and `/edikt:sdlc:drift` for pre-flight specialist review. Reference it instead of duplicating the routing logic.
...
### Fallback: agent installed this session

The Agent tool's type registry is fixed at session start — an agent installed into `.claude/agents/` mid-session is not dispatchable via `subagent_type` until the next session. If a dispatch fails with `Agent type '<slug>' not found` AND `.claude/agents/<slug>.md` exists on disk, that is what happened. Recover by re-dispatching with `subagent_type: "general-purpose"` and a prompt that instructs the agent to FIRST Read `.claude/agents/<slug>.md` and adopt its entire content as its operating instructions (persona, lens, tool constraints, output format), THEN perform the original task prompt unchanged.

This is a fallback ONLY, never the primary dispatch — `subagent_type: <slug>` remains the primary dispatch model. When the fallback fires, tell the user: restarting the Claude Code session registers the agent natively.
