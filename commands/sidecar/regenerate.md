...
```
 ...
 Rule 2: display binary output verbatim; do not parse it. Only inspect the exit code.
...
End with `✅ COMPLETE — all regressions resolved` when the worklist is clear, or a one-line summary of the items still needing manual review.

Next: re-run `/edikt:gov:compile` after regenerating so the refreshed sidecars land in the compiled governance.
