...
### 0. Resolve the template root

Every step below that reads a shipped template MUST resolve the template root through this order — project-mode first, never a hardcoded global path (same order as `/edikt:upgrade` §0y-bis):
...
- **Member onboarding** (default — user is joining an existing project):
  ...
  **Steps 1–6 — Member environment checks:**
  ...
  6. **Git pre-push hook:**
     ...
     - ⚠️ if missing — offer to install from `$EDIKT_TEMPLATES/hooks/pre-push`
...
Per the relevant decision, edikt does NOT ship default templates that auto-install. Per the relevant decision, invariants are called "Invariant Records" (short form `INV`). Each project explicitly chooses its template during init (or writes its own), and the choice is committed to git as `.edikt/templates/<artifact>.md`.
...
5. **If user picks [1] Adapt**:
   - Generate `.edikt/templates/adr.md` with:
     ...
     - A trailing HTML comment pointing at the co-located sidecar contract: `<!-- Compiled directives live in the co-located ADR-NNN-{slug}.edikt.yaml sidecar. edikt never writes to this .md — edit prose only; run /edikt:adr:compile to regenerate the sidecar. -->` (NO in-body `[edikt:directives:start]` block — v0.6.0 removed in-body sentinels)
...
6. **If user picks [2] Start fresh**:
   ...
   - Copy the chosen reference file to `.edikt/templates/adr.md` verbatim:
     - For [1] Nygard-minimal: copy `$EDIKT_TEMPLATES/examples/adr-nygard-minimal.md`
     - For [2] MADR-extended: copy `$EDIKT_TEMPLATES/examples/adr-madr-extended.md`
...
Per the relevant decision, invariants in edikt are formally called **Invariant Records** (short form `INV`).
...
6. Start fresh: show reference template choice:
   ...
   - For [1] Minimal: copy `$EDIKT_TEMPLATES/examples/invariant-minimal.md`
   - For [2] Full: copy `$EDIKT_TEMPLATES/examples/invariant-full.md`
...
5. Start fresh: reference template choice:
   ...
   Choice [1]:
   ```
   Copy from `$EDIKT_TEMPLATES/examples/guideline-minimal.md` or `$EDIKT_TEMPLATES/examples/guideline-extended.md` to `.edikt/templates/guideline.md`.
...
**Rules** — read the registry at `$EDIKT_TEMPLATES/rules/` to get the full list. Group by tier:
...
**Agents** — read the registry at `$EDIKT_TEMPLATES/agents/` to get the full list:
...
```
  ...
  ✓ Gitignore       .edikt/.gitignore (runtime state coverage)
...
features:
  ...
  evidence-gate: warn      # completion-claim gate posture:
                           # block | warn | educate | disabled. Default warn.

# Post-flight review composition.
# Auto-fires from phase-end-detector.sh after L1 PASS; composes L1 (criteria
# verify) + L2 (governance verifier) + L3 (specialist routing) + synthesis.
# Kill-switch env var: EDIKT_DISABLE_POST_FLIGHT=1 (overrides config).
post-flight:
  enabled: true
  specialists:
    required: []        # Always include these in the L3 set, even if auto-detection misses
    auto: ['*']         # Auto-detect from file-pattern routing in _shared-agent-routing.md
    never: []           # Always exclude — overrides auto AND required
...
# Gate severity thresholds per agent domain. Override via EDIKT_GATE_SEVERITY_THRESHOLD.
...
**`.edikt/.gitignore`** — Write this nested gitignore so runtime state (compile history, orphan-ADR caches, evaluator verdicts) never gets committed. Git honors nested `.gitignore` files, so this keeps the project root `.gitignore` clean and covers every edikt project automatically. Do NOT overwrite if the file exists.

```
# edikt-managed gitignore — runtime state generated by edikt commands.
# Safe to commit this file itself; the paths below are ephemeral.
state/
```
...
# Base rules:      $EDIKT_TEMPLATES/rules/base/{name}.md
# Language rules:  $EDIKT_TEMPLATES/rules/lang/{name}.md
# Framework rules: $EDIKT_TEMPLATES/rules/framework/{name}.md
#
# Example: to install the "go" rule pack:
#   Read: $EDIKT_TEMPLATES/rules/lang/go.md
#   Write to: .claude/rules/go.md
#
# Example: to install the "code-quality" rule pack:
#   Read: $EDIKT_TEMPLATES/rules/base/code-quality.md
...
```
Rule template not found: $EDIKT_TEMPLATES/rules/{tier}/{name}.md
Install edikt globally: curl -fsSL https://raw.githubusercontent.com/diktahq/edikt/main/install.sh | bash
```

**`CLAUDE.md`** — Read the template from `$EDIKT_TEMPLATES/CLAUDE.md.tmpl`. Sentinel merge using `[edikt:start]` / `[edikt:end]` markers. Fill template variables from config.
...
**`.claude/settings.json`** — Read the template from `$EDIKT_TEMPLATES/settings.json.tmpl`. The template contains the placeholder `${EDIKT_HOOK_DIR}` which MUST be substituted before writing — Claude Code does NOT expand environment variables in `command:` strings, so unsubstituted placeholders make hooks fail with `/bin/sh: /<hook>.sh: No such file or directory`.
...
```bash
# Agent template path: $EDIKT_TEMPLATES/agents/{name}.md
# Write to: .claude/agents/{name}.md
#
# Example: to install the "architect" agent:
#   Read: $EDIKT_TEMPLATES/agents/architect.md
#   Write to: .claude/agents/architect.md
```

For each enabled agent from step 3, follow this install sequence — do not shortcut:

**a. Load substitutions** — Read `$EDIKT_TEMPLATES/agents/_substitutions.yaml` once (reuse for all agents). It maps config path keys to their default path strings.
...
- macOS: `md5 -q $EDIKT_TEMPLATES/agents/{name}.md`
- Linux: `md5sum $EDIKT_TEMPLATES/agents/{name}.md | awk '{print $1}'`
...
### 4b. Generate Sidecars for Seeded/Adopted Artifacts (v0.6.0+)

After all artifacts are written or imported (steps 2a, 2a.2, and any bootstrap example artifacts seeded above), generate the co-located `<artifact>.edikt.yaml` sidecar for each artifact that does not already have one. v0.6.0 stores generated directive metadata in sidecars — `/edikt:gov:compile` refuses to run if any artifact lacks a sidecar.

On a fresh install the `sidecar-extractor` agent was written to `.claude/agents/` moments ago in step 4, so its `subagent_type` will not resolve in this session — expect these dispatches to use the fallback in `commands/_shared-agent-routing.md` § "Fallback: agent installed this session". This is normal on init.
...
What changed:
  ...
  Note: Agents register at session start — restart Claude Code to
        dispatch them natively. Until then, commands fall back
        automatically.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
- Single next step: start building. No list of 4 equal alternatives. The agent-restart caveat note is the one permitted addition.
