...
If a relevant agent is not installed (no `.claude/agents/{slug}.md` on disk):
```
DBA — not installed (run /edikt:agents add dba)
```

If the agent file exists but the dispatch could not resolve it (installed this session — the fallback from `commands/_shared-agent-routing.md` was used):
```
DBA — installed this session — fallback used; restart Claude Code to register natively
```

### Drift Check Output Format

```
DRIFT CHECK — SPEC-NNN
