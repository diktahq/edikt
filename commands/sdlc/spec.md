...
effort: high
argument-hint: "<PRD identifier, e.g. PRD-NNN>"
allowed-tools:
  - Read
  - Write
  - Edit
  ...
  - Agent
---
!`${HOME}/.edikt/bin/edikt next-id spec`
...
- `$ARGUMENTS` — PRD identifier (e.g., `PRD-NNN`) or path to the PRD file
...
```bash
find {BASE}/product/prds/ -name "PRD-NNN*" -type f   # e.g. PRD-NNN
```
...
```bash
find {BASE}/brainstorms/ -maxdepth 2 -name "BRAIN-NNN*" -type f 2>/dev/null | head -3
```
...
- **v2 PRD (sidecar exists):** The sidecar is source of truth for FRs, ACs, protections, and cross-references. Apply the described in Step 7b.
...
- `[required] The codebase has 3 ADRs about error handling. Should this spec follow ADR-NNN (wrapped errors) or propose a different approach?`
...
Route to `architect` + relevant domain specialists via the Agent tool. If a dispatch fails with `Agent type '<slug>' not found` but `.claude/agents/<slug>.md` exists (installed this session), use the fallback in `commands/_shared-agent-routing.md` § "Fallback: agent installed this session".
...
# Source traceability — exactly one of these three fields is set:
...
### 7b. Source-aware coverage and traceability
...
Read every `FR-NNN` from the PRD sidecar `requirements:` (FR ids follow the project's configured scheme — `ids.fr_pattern` in `.edikt/config.yaml`, default `FR-NNN`). For each FR, the spec must do one of:
...
Each AC MAY also carry an optional `verify:` field — a single shell command run by `bin/edikt verify spec <SPEC-ID>` (exit 0 = AC passes). `/edikt:sdlc:drift` folds verify failures into its drift signal. If the upstream PRD AC already carries a `verify:`, pass it through unchanged. For new spec-only `SAC-NNN`, prompt the user:

```
How would you verify {SAC-NNN} is satisfied? Give a shell command that
exits 0 when the architectural check holds (e.g., a migration smoke test,
a contract diff, or a static-analysis invocation). Press Enter to skip if
no programmatic check exists.
```

The same prompt is offered for each `SR-NNN` you author at this layer:

```
How would you verify {SR-NNN} is satisfied at the architecture layer?
Give a shell command that exits 0 when the spec requirement holds (e.g.,
a go vet rule, a contract diff, a build invocation). Press Enter to skip
if no programmatic check exists.
```

When the user skips, OMIT the field — never write `verify: ""`. Empty strings break the schema (`minLength: 1`).

```yaml
# In SPEC yaml front-matter or companion sidecar:
acceptance_criteria:
  # From PRD (pass-through, unchanged — verify: copied verbatim if present):
  - id: AC-001-1
    fr: FR-001
    given: "..."
    when: "..."
  ...
  - id: SAC-001
    ...
    verify: "bash test/integration/migration_rowcount.sh"
```
...
Recompute `_sync` hashes (SHA-256 via `python3 -c 'import hashlib,sys; print(hashlib.sha256(open(sys.argv[1],"rb").read()).hexdigest())'` with argv,). Then Edit the sidecar's `_sync:` block with the new hashes and ISO8601 timestamp.
...
### 7c. Render the sidecar from `templates/spec-sidecar.yaml.tmpl`

The spec sidecar (`{specs_dir}/SPEC-{NNN}-{slug}/spec.yaml`) is rendered from a template — the same way `/edikt:sdlc:prd` renders the PRD sidecar. Do NOT hand-roll YAML inline.

**MANDATORY — read the template first, render it whole.** Use the Read tool on `templates/spec-sidecar.yaml.tmpl` (resolve via `$EDIKT_HOME/current/templates/spec-sidecar.yaml.tmpl` or `$HOME/.edikt/current/templates/spec-sidecar.yaml.tmpl`). Replace `{{placeholder}}` tokens with values; preserve every other key verbatim. Do NOT compose the sidecar from memory or pattern-match from an existing SPEC — the template is the contract.

**Required top-level fields (per `templates/schemas/spec-sidecar.v1.schema.json`):**

| Field | Source |
|---|---|
| `schema_version` | always `"1.0"` |
| `type` | always `"spec"` |
| `id` | `SPEC-NNN` from the live-injected next number |
| `title` | the user-confirmed title |
| `slug` | kebab-case of the title |
| `status` | always `"draft"` for a new SPEC |
| `author` | output of `git config user.name` |
| `created_at` | ISO8601 UTC timestamp computed at write time |

**Source discriminator (oneOf — exactly one non-null):**
- `source_prd` ← when `$SOURCE_TYPE == prd` (e.g., `PRD-NNN`)
- `source_brainstorm` ← when `$SOURCE_TYPE == brainstorm` (e.g., `BRAIN-NNN`)
- `source_prompt` ← when `$SOURCE_TYPE == prompt` (truncated prompt string, ≤200 chars)

Set the two unused source fields to `null`. The schema's `oneOf` constraint rejects sidecars where zero or multiple source fields are non-null.

**Required collection fields (may start empty but key MUST be present):** `requirements`, `acceptance_criteria`, `protections`, `solution_references`, `revision_history`, `extensions`, `_sync`.

**Required nullable fields:** `supersedes`, `superseded_by`, `deprecated_at`, `deprecated_reason`, `cancelled_at`, `cancelled_reason` (set to `null` for a new SPEC).

**Optional fields populated from upstream:**
- `source_prd_coverage` — emitted only when `source_prd` is set AND PRD is v2 (see Change 1 above for shape)

**Validation gate before write.** After rendering the sidecar in memory, validate it against `templates/schemas/spec-sidecar.v1.schema.json`:

```bash
python3 <<'PYEOF' "$SPEC_YAML_RENDERED" "$SCHEMA_PATH_ABSOLUTE"
...
If validation fails: do NOT write the sidecar. Show the user which fields are missing/invalid, fix in-memory, re-validate, then write.

If `jsonschema` is not installed, fall back to a structural check: assert all 8 required top-level fields are present AND exactly one of `source_prd / source_brainstorm / source_prompt` is non-null.

**Computing `{{schema_path}}`:** Compute the relative path from the SPEC's parent directory to `.edikt/schemas/spec-sidecar.v1.schema.json`:

```bash
SCHEMA_PATH=$(python3 -c 'import os,sys; print(os.path.relpath(sys.argv[2], sys.argv[1]))' "$(dirname "$SPEC_YAML")" "$PROJECT_ROOT/.edikt/schemas/spec-sidecar.v1.schema.json")
```

If `.edikt/schemas/spec-sidecar.v1.schema.json` does NOT exist in the project, auto-install it as part of this step:

```bash
test -f "$PROJECT_ROOT/.edikt/schemas/spec-sidecar.v1.schema.json" || {
  mkdir -p "$PROJECT_ROOT/.edikt/schemas"
  SOURCE_SCHEMA="$EDIKT_HOME/current/templates/schemas/spec-sidecar.v1.schema.json"
  [ -f "$SOURCE_SCHEMA" ] || SOURCE_SCHEMA="$HOME/.edikt/current/templates/schemas/spec-sidecar.v1.schema.json"
  cp "$SOURCE_SCHEMA" "$PROJECT_ROOT/.edikt/schemas/spec-sidecar.v1.schema.json"
}
```

This bootstrap keeps the IDE autocomplete working on first-ever SPEC creation without requiring a separate install step.
...
✅ Spec created: {specs_dir}/SPEC-{NNN}-{slug}/spec.md
  ...
  Re-score:     /edikt:sdlc:spec-review SPEC-{NNN}
```
