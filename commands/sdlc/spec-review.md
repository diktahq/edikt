...
- FR coverage completeness (every PRD FR-NNN is covered or explicitly deferred — the FR ID scheme may be customized via `ids.fr_pattern` in `.edikt/config.yaml`)
- AC pass-through integrity (every PRD AC-NNN-M appears in the SPEC unchanged)
- Broken refs + drift (same pattern as `/edikt:sdlc:prd-review`)
...
```
Usage: /edikt:sdlc:spec-review SPEC-NNN
```
And stop.
...
SPEC REVIEW — SPEC-NNN
  ...
  ⚠ Broken references ({count}):
    • ADR-942 — not found in {decisions_dir}/
...
Next:
  ...
  • Review linked PRD:   /edikt:sdlc:prd-review PRD-NNN
...
Recompute `_sync` hashes (SHA-256 via python3 argv — compliant).
...
End with a one-line verdict, e.g. `✅ SPEC-NNN reviewed: rubric 18/20, coverage 12/12, AC pass-through 12/12`.
