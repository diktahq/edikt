...
allowed-tools:
  ...
  - Agent
---
!`${HOME}/.edikt/bin/edikt next-id prd`
...
PRDs evolve via edit-in-place. Lifecycle transitions (ship, cancel, deprecate, supersede) are handled by this same command — see [Lifecycle Transitions](#lifecycle-transitions) below.
...
- `PRD-NNN supersede` — supersede with a new PRD (≥50% scope change,)
...
- Evaluator mode: `evaluator.mode` (default: `headless`,)
...
1. **Matches `PRD-\d+\s+(ship|cancel|deprecate|supersede)`** (or natural-language equivalent: "ship PRD-NNN", "cancel PRD-NNN — budget cut") → **lifecycle transition mode**
...
**FR numbering:** Start at `FR-001`, sequential. Each FR is a single testable statement. This default scheme is configurable via `ids.fr_pattern` in `.edikt/config.yaml` — projects with component-coded IDs (e.g. `FR-PR-001`) set a custom pattern there.

**AC numbering:** `AC-NNN-M` where NNN = FR number (exact match, not sequential within AC list), M = criterion index within that FR starting at 1. So a FR-001 with 2 ACs has `AC-001-1` and `AC-001-2`. Like FRs, the AC scheme is configurable via `ids.ac_pattern`.

**Given/When/Then:** Every AC uses this format. Each AC MAY also carry an optional `verify:` field — a single shell command run by `bin/edikt verify prd <PRD-ID>` (exit 0 = AC passes). Lifecycle transitions like `ship` shell to that runner first and refuse to advance status when any verify fails.
...
**Interview for `verify:` (per FR and per AC).** When drafting each FR and each AC, ask the user:

```
How would you verify {ID} is satisfied? Give a shell command that exits 0
when the requirement holds (e.g., a test runner, integration script, or
grep). Press Enter to skip if no programmatic check exists.
```

Capture the answer as `verify:` on the FR (for the requirement-level check) or on the AC (for the Given/When/Then-level check). When the user skips, OMIT the field — never write `verify: ""`. Empty strings break the schema (`minLength: 1`) and fabricated commands defeat the discipline.

The same prompt is offered for each FR (`verify` field on the requirements[] entry) and each AC. FRs and ACs without a verify still validate; they just can't be auto-checked by lifecycle gates and surface in the doctor's verify-coverage report as gaps.
...
I found {N} existing invariants that may apply to this PRD:
  ...
  [1] INV-NNN — Hook JSON emission
      Relevant because: FR-002 mentions hook behavior

  [2] INV-NNN — Input shape validation
...
If **missing**: use the Write tool to create `.edikt/rubrics/prd.md` with the default rubric content below. This is a mandatory bootstrap step — do NOT proceed to scoring without the file on disk (otherwise `/edikt:sdlc:prd-review` runs later will re-bootstrap and the score history becomes inconsistent).
...
_Users can edit this rubric template overrides._
```
...
```bash
# Headless evaluator invocation compliant
...
Fall back to in-session reasoning if headless fails (visible warning).
...
Render `{prd_yaml}` from `templates/prd-sidecar.yaml.tmpl` with the full structured content.

**MANDATORY — read the template first, render it whole.** Use the Read tool on `templates/prd-sidecar.yaml.tmpl` (resolve via `$EDIKT_HOME/current/templates/prd-sidecar.yaml.tmpl` or `$HOME/.edikt/current/templates/prd-sidecar.yaml.tmpl`). Replace `{{placeholder}}` tokens with values; preserve every other key verbatim. Do NOT compose the sidecar from memory or pattern-match from an existing PRD — the template is the contract.
...
```bash
# Use python3 with argv for safe relative-path computation
...
For a PRD at `docs/product/prds/PRD-NNN-x.yaml` with default layout, `SCHEMA_PATH` resolves to `../../../.edikt/schemas/prd-sidecar.v1.schema.json`.
...
Next steps:
  ...
  • Re-score anytime:           /edikt:sdlc:prd-review PRD-{NNN}
...
This command creates PRDs in `status: draft`. Lifecycle transitions (ship, cancel, deprecate, supersede) are dispatched via this same command — pass the verb after the PRD identifier (e.g. `/edikt:sdlc:prd PRD-NNN ship FR-001`). Supersession is rare — reserved for ≥50% scope rewrites.
...
- `/edikt:sdlc:prd-review` — re-score the PRD against the rubric
...
Next: Review the PRD with /edikt:sdlc:prd-review to score it against the rubric, then run /edikt:sdlc:spec to generate the technical spec.
...
**Parse FRs to ship:** Resolve the FR ID pattern from `ids.fr_pattern` in `.edikt/config.yaml` (default `^FR-\d{3,}$`) and match it against each argument token — drop the `^`/`$` anchors as appropriate when scanning tokens embedded in longer argument text. If the user passed arguments beyond the PRD ID but none match, ERROR — do NOT fall through to the interactive list:

```
⛔ No FR IDs recognized in the arguments.
Tokens {non-matching tokens} don't match the active FR ID pattern `{pattern}`.
If this project uses a custom ID scheme, check ids.fr_pattern in .edikt/config.yaml.
```

If no FR tokens were given at all (bare `ship`), list unshipped FRs and ask:
...
**Pre-transition verify gate.** Before mutating the sidecar, run the completion-evidence gate against the PRD's current state:

```bash
bin/edikt verify prd PRD-NNN
```

- Exit 0 → proceed to the mutation below.
- Exit 1 → refuse the ship. Surface the per-item failures and a remediation:

```
⛔ Cannot ship PRD-NNN — {failing_n} of {m} verify(s) failed:
  FR-NNN: exit=1 — `<failing command>`
  AC-NNN-M: exit=1 — `<failing command>`
Fix the failing requirements, or edit the sidecar to remove the bad
verify command. Then re-run /edikt:sdlc:prd PRD-NNN ship.
```

Ship stays pending — the sidecar is not mutated. Do NOT auto-edit verifies away to force the ship through.
...
Supersede is reserved for ≥50% scope rewrites or problem-framing shifts. For routine FR changes, use continuation or ship/cancel/deprecate instead.

**Guard:** If old PRD is already `superseded` or `cancelled` → error "PRD-NNN is already {status}; cannot supersede."

**Gate on ≥50% threshold:**

Show what will be lost:

```
⚠️  Supersede is a heavy transition.
...
**Pre-transition verify gate.** Before authoring the new PRD, run the completion-evidence gate against the OLD PRD:
...
- Exit 0 → proceed.
- Exit 1 → refuse the supersession unless `--force-verify` is set. A PRD with failing verifies has unfinished work; superseding hides that work behind the new PRD's FR numbering. Either fix the failing FRs/ACs first, or pass `--force-verify` to acknowledge you're retiring the failures alongside the supersession (revision_history records the override).
...
Next:
  • Review new PRD:  /edikt:sdlc:prd-review PRD-MMM
