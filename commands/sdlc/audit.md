...
6. Spawn TWO agents in parallel via the Agent tool:
   ...
   If a dispatch fails with `Agent type '<slug>' not found` but `.claude/agents/<slug>.md` exists, use the fallback in `commands/_shared-agent-routing.md` § "Fallback: agent installed this session".
