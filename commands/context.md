...
**focused depth:** Load only PRD and SPEC sidecars referenced in the active plan phase (plan-scoped context loading). This keeps context budget lean — the active plan is the signal for what is being worked on now.
...
{list of ADR titles with status — one line each, e.g. "ADR-NNN (Accepted) — Use PostgreSQL for persistence"}
