...
- The user-facing `edikt` binary MUST be the Go binary in `tools/gov-compile/`. `bin/edikt` (bash launcher) is deleted. `install.sh` places only the Go binary. (ref: ADR-022)
- Subcommands not yet in Go MUST be delegated to `bin/edikt-shell` via os/exec with full stdio passthrough. When `bin/edikt-shell` is absent, emit a clear error and exit non-zero. NEVER hang. (ref: ADR-022)
- Compiled binary artifacts MUST NOT be committed to git. Shipped only via release assets (ADR-016), excluded by .gitignore. (ref: ADR-022)
