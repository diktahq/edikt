...
<!-- source: 18 ADRs (18 accepted, 3 superseded), 8 invariants (8 active), 0 guidelines -->
<!-- compiled_by: edikt v0.5.0 -->
<!-- compiled_at: 2026-04-17T00:00:00Z -->
<!-- directives: 102 across 7 topic files | tokens: ~2700 -->
...
| agent, specialist, advisor, subagent, evaluator, domain signal, BLOCKED, verdict schema | planning, design, review, implementation | `governance/agent-rules.md` |
| hook, PostToolUse, format, preprocessing, CLAUDE.md, sentinel, fixture, characterization, JSON protocol, systemMessage, permissions, settings.json | implementation | `governance/hooks.md` |
...
| release, install, SHA256SUMS, cosign, signing, Sigstore, integrity, tag, upgrade | implementation, design, review | `governance/release.md` |
| tier-2, helper, tools/, deterministic, gov-compile, latency, token cost | implementation, design | `governance/tooling.md` |
...
- Hook scripts MUST emit protocol JSON via a structured serializer, never shell string concatenation. (ref: INV-003)
- Hooks MUST NOT instruct Claude to execute shell commands built from agent-derived or untrusted text. (ref: INV-004)
- Managed regions MUST be guarded by byte-range checks (markdown) or sidecar hash (JSON), NEVER regex-over-patch. (ref: INV-005)
- Externally-controlled inputs MUST be allowlist-validated before interpolation into argv, URLs, or prompts. (ref: INV-006)
- Benchmark/test sandboxes MUST be hermetic — no host hooks, no user settings_sources. (ref: INV-007)
- Release install URLs MUST be tag-pinned, never branch-tracking. (ref: INV-008)
