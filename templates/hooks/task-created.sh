...
# Every extraction and the event write below need python3. Absence used to
# fail every step silently via `|| true`/`|| pass` and still print a bare
# {"continue": true} — indistinguishable from a healthy run, so task-created
# events vanished from the ledger with nothing anywhere to say so.
if ! command -v python3 >/dev/null 2>&1; then
    echo "edikt: task-created — python3 not found; this task was not logged (plan-phase attribution lost)" >&2
    printf '{"continue": true, "systemMessage": "⚠ edikt: python3 is missing — task creation was not logged. Install python3 to restore task/plan-phase tracking."}\n'
    exit 0
fi
...
for dir in "$PLANS_DIR" "$BASE/product/plans" "docs/product/plans"; do
    [ -d "$dir" ] || continue
    # F-059 (D23): stat -f %m F || stat -c %Y F is broken on GNU (see
    # status-line.sh for the full explanation). date -r reads mtime  edikt-guard:allow
    # identically on BSD and GNU.
    CAND=$(find "$dir" -maxdepth 1 -type f -name 'PLAN-*.md' 2>/dev/null | while read f; do
        printf '%s\t%s\n' "$(date -r "$f" +%s 2>/dev/null || stat -f %m "$f" 2>/dev/null)" "$f"
