...
#   edikt_log_event "status_change" '{"artifact":"SPEC-NNN","from":"draft","to":"accepted"}'
...
edikt_log_event() {
  ...
  # This is a sourced shell function, not a standalone hook — it has no hook
  # protocol output of its own. Absence used to fail the json.dumps call
  # below silently via `|| return 0`, so every caller across the hook tier
  # (gate firings, task/worktree telemetry) lost its record with nothing
  # anywhere to say so. stderr is the only channel this function has.
  if ! command -v python3 >/dev/null 2>&1; then
    echo "edikt: event-log — python3 not found; event '${event_type}' was not recorded" >&2
    return 0
  fi
