...
if [ -f "$CACHE_FILE" ]; then
    # F-059 (D23): the previous form was `stat -f %m F || stat -c %Y F`. On
    # BSD, -f is the FORMAT flag and it works. On GNU, -f means FILESYSTEM  edikt-guard:allow
    # status, %m is not a valid filesystem specifier, so it prints "?" and
    # exits 0 — the || never falls through. `date -r` reads mtime identically
    # on both platforms (see session-start.sh, event-log.sh for the same fix).
    AGE=$(( $(date +%s) - $(date -r "$CACHE_FILE" +%s 2>/dev/null || stat -f %m "$CACHE_FILE" 2>/dev/null || echo 0) ))
