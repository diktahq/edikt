...
#                           "additionalContext": "..."}}
# Migration: plaintext echoes wrapped in JSON, content preserved byte-for-byte.
...
# Every emit() call below needs python3. Absence used to fail at rc=0 with
# ZERO bytes of stdout — the healthy-vs-degraded output was indistinguishable
# and 238/238 SessionStart fires already showed this class can go unnoticed
# for a long time (F-044). A constant literal — nothing interpolated, no
# json.dumps needed.
if ! command -v python3 >/dev/null 2>&1; then
    echo "edikt: session-start — python3 not found; session summary and gate-finding check skipped" >&2
    printf '{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"⚠ edikt: python3 is missing on this host — the session summary (changed files, relevant agents, memory age) and last-session gate-finding check could not run. Governance rules still load normally; only this summary is affected."}}\n'
    exit 0
fi
...
#
# F-044 / ADR-061 §4: this emitted a BARE {"additionalContext": ...}. The host  edikt-guard:allow
# accepts that, and DROPS it. 238 of 238 recorded SessionStart fires delivered
# nothing, completely silently — the payload has to sit inside
# hookSpecificOutput with its hookEventName to reach the model.
emit() {
...
# Surface most recent unresolved gate finding.
# Only the single most recent — do not list all. Only if events.jsonl exists and
# contains an unresolved gate_fired. Exits 0 silently on any error.
...
PY
  2>/dev/null)
  if [ -n "$GATE_MSG" ]; then
    python3 -c 'import json,sys; print(json.dumps({"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":sys.argv[1]}}))' "$GATE_MSG"
    exit 0
  fi
fi
