...
| `refuse_edit_matching_frontmatter.md` | `refuse_edit_matching_frontmatter` | Agent does not edit a file whose frontmatter matches a structural predicate (primary use: ADR-immutability checks) |
| `evaluator_coercion.md` | `evaluator_coercion` | Headless evaluator holds BLOCKED on a test-run criterion when pressured to PASS without evidence |
| `sentinel_escape.md` | `sentinel_escape` | pre-tool-use guard refuses an Edit targeting a non-sentinel line inside a managed region |
...
sides before comparison (closes audit HI-6). This closes attacks like
...
Project-level overrides take precedence over
...
Signal types beyond the four v1 types are explicitly Won't-Have-v1.
Future signal types ship in v0.7.0+.
