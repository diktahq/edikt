...
**Action:** read every sidecar, validate against the schema, group by topic, and render the four output surfaces of `compile_schema_version: 3` (the ambient core, per-topic files, the directive index, and the manifest — see [The four-surface render](#the-four-surface-render-compile-schema-version-3) below). This phase is a pure deterministic merge. No LLM, no `Task`/`Agent` dispatch, no shell-out.
...
`compile_schema_version: 3` is the current output-format contract of `/edikt:gov:compile`, defined in ADR-059 (amending ADR-007, which introduced the schema-version field as a way to let tooling ask "was this produced by a compatible compile" without conflating that with edikt's own marketing version). Schema 3 replaced a single always-loaded `governance.md` with four distinct rendered surfaces, each doing one job:
...
- **Scoped topic files** — `.claude/rules/governance/<topic>.md`, one per topic, loaded automatically by Claude Code's own path-matching when the frontmatter `paths:` glob matches a touched file. Each carries a `Directives`/`Prohibitions` region (may be empty — see [ADR-066's single-home rule](#adr-066-s-single-home-rule-scoped-directive-text-renders-exactly-once) below), a hand-authored `ManualDirectives` region (may also be empty), and always-present `Reminders` and `Verification Checklist` sections built from that topic's directives. A topic whose contributing sidecars declare no `paths:` at all renders no topic file — it's reached only through its skill package instead.
- **The directive index** — `.claude/rules/governance/directive-index.yaml`. A single glob-keyed YAML file, not meant for ambient reading — it is `bin/edikt hook match`'s exclusive input, read by the write-time hook shims (see [The write-time split](#the-write-time-split-adr-060-what-happens-to-a-directive-when-a-write-happens) below). Each entry under a glob key carries an `id`, a pinned `grade` (`must` or `advisory`, decided once at compile time — consumers never re-derive it), the literal directive `text`, a `falsifying_observation`, and its `reminders`. A single directive can appear under every glob it matches, not once per directive.
...
A reader can tell which surface they're looking at from its shape alone: the ambient core has `paths: "**/*"` and almost no body; a topic file has a topic-scoped `paths:` list, a `Directives`/`Prohibitions`/`ManualDirectives` triad, and always a Reminders + Verification Checklist tail; the directive index is pure YAML keyed by glob, never prose; the manifest is a flat list of `path` + `kind` + `sha256` triples. Per-topic skill packages at `.claude/skills/edikt-<topic>/SKILL.md` are also listed in `manifest.yaml` (`kind: "skill-package"`, one entry per topic) alongside the ambient core, topic files, and directive index — four kinds of entry, one manifest. A skill package is Claude Code's trigger-loaded surface — read in full on task-language selection rather than passively included by path — and it is where a fully-unscoped topic's directives live when no topic file exists to carry them. No rendered surface carries a "compiled at" timestamp field; that field was deliberately removed under ADR-020's byte-determinism requirement, because a timestamp turns every no-op compile into a diff.

## ADR-066's single-home rule: scoped directive text renders exactly once

Schema 3 shipped with a duplication bug, closed by ADR-066 (2026-08-15) and measured on a real 50-sidecar consumer project: 78 of 110 directive lines in that project's topic files appeared verbatim, a second time, in `directive-index.yaml` — both surfaces scoped to overlapping `paths:` globs, both firing on the same edit.

The two surfaces are not accidental duplicates of each other; each is a real, independent delivery channel with its own contract. `directive-index.yaml` is `hook match`'s exclusive input — it fires precisely on the file actually being touched, matched per-directive against that directive's own glob, and per ADR-060 it drives the write-time hooks (a synchronous PreToolUse deny for `must`-grade directives, a PostToolUse `additionalContext` for `advisory`-grade ones). The topic file, by contrast, is Claude Code's native ambient rule-loading surface: the whole file loads into context whenever a touched path matches the topic's `paths:` frontmatter, independent of the hook — passive background context rather than a guaranteed, synchronous delivery. Before ADR-066, a scoped directive was delivered by both, and the second delivery added token cost without adding coverage, because the hook's guarantee is strictly the stronger of the two.

The precise rule: **"scoped" means the sidecar declares a `paths:` field.** A sidecar's `Directives` and `Prohibitions` text contributes to its topic group's rendered body — the topic file's compiled-directives region — only when that sidecar declares no `paths:`. A sidecar that does declare `paths:` contributes its directive and prohibition text to `directive-index.yaml` alone, and the topic file's compiled-directives region omits it entirely. As one worked example, in edikt's own tree every one of the six rendered topic files currently has an empty compiled-directives region — open/close sentinel markers back to back with no content between them — because every sidecar contributing to those topics declares `paths:`, so all of their directive text lives in `directive-index.yaml` instead. If your project sees the same thing, that emptiness is not a bug and not a sign the corpus is thin; it's the rule working as designed on a corpus where scoping is thorough.

What stays in the topic file, regardless of this rule, is pathless/unscoped content and hand-authored `ManualDirectives`. A sidecar that declares no `paths:` still contributes its directive text to the topic file's body — there's nowhere else for it to reach a reader by path-match, since it has no glob for the hook to key on either. `ManualDirectives` are unaffected by the `paths:` question entirely: hand-authored by a human directly into the sidecar, never extracted, no other delivery channel, they render into the topic file's manual region regardless of whether that sidecar also declares `paths:`. The topic file's own `paths:` frontmatter is likewise unaffected: it remains the union of every contributing sidecar's declared globs, so Claude Code's ambient loader keeps surfacing the file — now carrying only pathless/manual content, where present — on a matching touch. ADR-066 does not go further and suppress rendering a topic file whose compiled-directives region is empty; that's a smaller, separable optimization left for later.

## The write-time split (ADR-060): what happens to a directive when a write happens

ADR-060 (2026-08-12) closes a gap schema 3's ambient-token reduction opened: a directive scoped to, say, `tools/**/*.go` only reaches a reader when the host happens to load that topic file, which depends on the host's own rules-loading behavior, not on the write actually happening. That's acceptable for an advisory rule. It is not acceptable for a `must`-grade rule, because the entire point of an invariant is that it applies whether or not anyone remembered to load it. ADR-060's answer is a channel that fires *because a write is happening*, keyed on the file being written, independent of ambient loading — two hooks over one matcher, split by grade:
...
A `must`-grade directive is enforced *before* the write completes: the PreToolUse hook blocks the tool call with a deny that names the specific matched directive, and the write does not go through until the agent complies. An `advisory`-grade directive is delivered *after* the write completes: the PostToolUse hook injects `additionalContext` carrying the directive text, but nothing was blocked — the write already happened, and the injected text is guidance for what comes next, not a gate on what just occurred. This split is grounded in a direct behavioral measurement (BRAIN-005 E1): advisory text appended after the fact was read and largely ignored, while a deny naming the rule and forcing a retry was complied with. The two paths are kept disjoint deliberately: a `must` rule that also appeared in advisory context would be delivered twice, and the weaker of the two deliveries is the one an agent learns to route around.

The grade itself (`must` vs. `advisory`) is decided once, at compile time, and pinned directly into `directive-index.yaml`'s `grade` field — consumers of the index never re-derive it (grade derivation is keyed off the directive's actual modal verb, not a keyword scan, per ADR-064). The write-time tier is deliberately fail-open rather than fail-closed: this code runs inside PreToolUse, and failing closed would mean a governance bug blocks every write in the user's editor until someone diagnoses it — a worse outcome than a missed injection. But fail-open must never be *silent* fail-open: several distinct failure classes (binary absent, index missing, index corrupt, path unresolvable) produce the exact same observable as a correct "no directive applies here" pass — a write that proceeds with nothing injected — so every invocation is journaled with a distinctly-typed outcome. This tier only covers `file_path`-bearing tool calls — a write performed through Bash (redirection, `tee`, in-place stream editing) carries no `file_path` and is not matched by this tier at all, a named and accepted gap, not a silent one.
...
1. **Non-Negotiable Constraints** — the invariant-derived rules that held everywhere — the same role the current ambient core's single constraint plays, but at schema 2 this section wasn't the *only* always-loaded content; the next three sections loaded on every edit too.
2. **Routing Table** — a markdown table, one row per topic, each row listing a long comma-separated block of keyword/phrase "Signals" (hundreds of terms — the architecture-topic row alone carried well over 90 keywords), a "Scope" column, and the topic file path. Measured directly: this table alone ran to roughly 629 keyword terms at approximately 3,119 estimated tokens, loaded on every single edit regardless of relevance, and was measurably less precise than the one-line topic descriptions that replaced it (BRAIN-005 E4).
3. **Reminders** — a flat, aggregate list of every "Before doing X, verify Y" line across the entire corpus, always loaded, not scoped to the file being touched.
4. **Verification Checklist** — a flat, aggregate checklist covering every directive in the corpus, again always loaded regardless of relevance.

The bottom "Reminder: Non-Negotiable Constraints" block restated section 1's content a second time "for emphasis." Invariants appeared at the top (primacy bias) and bottom (recency bias).

Each retired piece was replaced by a specific schema-3 mechanism, not simply deleted: the keyword-matched Routing Table was replaced by the ambient core's one-line topic index combined with automatic `paths:`-based loading of topic files, and skill-package description matching for topics with no `paths:` at all. The aggregate Reminders and Verification Checklist sections were replaced by per-topic Reminders/Verification Checklist sections that render only inside the topic file a matching edit actually loads — the content didn't disappear, its scope shrank from "every edit" to "edits that match this topic." The bottom restatement block was dropped outright with nothing replacing it; the ambient core states its non-negotiable constraints exactly once. Any description of a single `governance.md` with a routing table, or of directives as always loaded regardless of the file being touched, is describing this retired schema-2 model.
