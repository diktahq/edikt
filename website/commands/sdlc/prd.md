...
|------|----------|--------|
| `ship FR-NNN ...` | Requirements have shipped | Marks FRs `shipped`. If all FRs ship, top-level status flips to `shipped`. **Gated on `verify prd`.** |
...
| `supersede` | ≥50% scope rewrite | Creates a new PRD-MMM, links both directions. **Breaks the stable-ID chain.** Also gated on `verify prd`. |

### Completion-evidence gate (ship + supersede)

Before mutating the sidecar, `ship` and `supersede` shell to:

```bash
bin/edikt verify prd PRD-NNN
```

The runner walks every `requirements[].verify` (FRs) and `acceptance_criteria[].verify` (ACs) in the PRD sidecar. Exit 0 = all pass or all skipped; exit 1 = at least one failed.

- **`ship` on failure** — refuses the transition. The sidecar is not mutated. Each failing item is surfaced with its captured stderr; remediation is to fix the underlying behaviour or edit the sidecar to remove the bad `verify:` line, then re-run `ship`.
- **`supersede` on failure** — refuses unless `--force-verify` is set. The override is recorded in `revision_history` so the audit trail captures that the new PRD was authored while the old one still had failing verifies.
- **`cancel` and `deprecate` are intentionally not gated** — you may need to deprecate a PRD precisely because its verifies are failing.

Items without a `verify:` field are recorded as `skipped:operational` and don't block the transition. The doctor's "Sidecar Verify Coverage" line surfaces gaps as soft warnings.

### Supersede gate (four yes/no questions)

Supersession is also gated on substantive scope change. The command asks four yes/no questions and refuses to proceed unless three or four are yes:
...
For routine changes, prefer continuation (`/edikt:sdlc:prd PRD-005`), shipping FRs (`ship FR-NNN`), or deprecation. Supersession is rare.
...
Every write recomputes `_sync.md_hash` and `_sync.yaml_hash` over the canonical content. When the `.md` is edited outside the command (typo fix, manual edit), the hash drifts. `/edikt:doctor` and `/edikt:sdlc:prd-review` flag the drift so it doesn't go unnoticed.
...
- [/edikt:sdlc:prd-review](/commands/sdlc/prd-review) — re-score the PRD against the rubric
