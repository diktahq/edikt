...
ACs from the PRD pass through unchanged — same `id`, byte-equal Given/When/Then. The SPEC may add architectural acceptance criteria as `SAC-NNN` but must not renumber, rewrite, or merge PRD ACs. Coverage is verified by [/edikt:sdlc:spec-review](/commands/sdlc/spec-review) and at phase-end by the evaluator.
...
- [/edikt:sdlc:spec-review](/commands/sdlc/spec-review) — re-score the SPEC and verify FR coverage + AC pass-through
