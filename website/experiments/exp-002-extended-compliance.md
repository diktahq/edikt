...
*Conducted 2026-03-21 · edikt v0.1.0 · Claude Sonnet 4 + Claude Opus 4 · 63 runs.* The result below is
permanently true of that configuration — the question it answers isn't version-specific, even though
edikt's delivery surface has changed since.
...
**What this means for edikt:** Rules are robust under stress. Multi-rule, multi-file, and cross-model all hold. The adversarial finding is important: edikt can enforce conventions when the user cooperates, but cannot override explicit user intent on non-security rules. This is correct behavior — governance should set defaults, not remove user agency.

**What we built from this:** v0.7.0's deny channel moves the non-security-rules resolution out of model discretion for the cases that matter most — a `must`-grade directive now blocks the write before it happens rather than relying on the model choosing to comply. The triage logic this experiment found (yield on arbitrary conventions, refuse on security) is still real for advisory-grade rules; for must-grade rules, enforcement no longer depends on it. See [Upgrading to v0.7.0](/guides/v0.7.0-upgrade).
