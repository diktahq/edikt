...
directives:
  ...
  - "ADDED BY USER: must enable TLS between app and Redis"
manual_directives: []
suppressed_directives: []
[edikt:directives:end]: #

<!--
This fixture represents the state where:
  - source_hash matches the current body (body is unchanged)
  - directives_hash does NOT match the current directives: list
    (because the user added "ADDED BY USER: must enable TLS..." by hand)

Expected behavior when /edikt:adr:compile runs on this file:
  - Interactive mode: interview triggers, asks the user what to do with
    the hand-added line. Options [1]-[5] from ADR-008 Section 5.
  - Headless mode: exits with error (2) and prints --strategy= overrides.
-->
